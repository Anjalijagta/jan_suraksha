...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($err === '') {
        ...
        if (!$err) {
            ...
            // Prepare INSERT statement with anonymous support and urgent flag
            $stmt = $mysqli->prepare('INSERT INTO complaints (user_id, complaint_code, complainant_name, mobile, crime_type, date_filed, location, description, evidence, status, is_anonymous, anonymous_tracking_id, is_urgent, urgency_justification, urgent_marked_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())');
            ...
            if ($stmt === false) {
                ...
                $stmt->bind_param('issssssssiisis', $uid, $code, $finalName, $finalMobile, $crime, $date, $location, $finalDescription, $uploadedFile, $status, $isAnonymousFlag, $anonymousTrackingId, $urgentFlag, $sanitizedJustification);
